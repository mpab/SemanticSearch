...
from utilities import CmdLineUtil, Folders, JsonEncoder, StringUtil
...
def tokenise_keywords(text) -> List[str]:
    ...
    # apply filter
    keywords = [\
        word for word in wordtokens if \
        not word in stopwordswords and \
        not word in punctuations and \
        not word in single_numbers and \
        not word in single_letters and \
        not word in symbols1 and \
        not word in symbols2 and \
        not word in symbols3]
...
def features_extract_by_identifier_hash_and_result_hash(identifier_hash: str, result_hash: str) -> Features:
    ...
    info = "extracted " + extract_count + " features with " + error_count + " errors from: " + context.friendly_name()
    print (info)
