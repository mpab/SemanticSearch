...
def features_extract_by_identifier_hash_and_result_hash(
    ...
    none_context = SearchContextExt.deserialize_from_identifier_hash(identifier_hash)

    if none_context is None:
        features.setInvalid("context not found")
        return features

    context: SearchContext = none_context
...
def features_extract_by_identifier_hash(identifier_hash: str) -> Tuple[int, str]:
    ...
    return (features.is_valid, features.info)


def context_features_extract_by_context_parameters(
    context_parameters: ContextArgs,
) -> Tuple[int, str]:
    request = SearchRequest(context_parameters.data_source, context_parameters.args)
    return features_extract_by_identifier_hash(request.identifier_hash)


def main():
    args: ContextArgs = Args.parse_cmdline()
    success, info = context_features_extract_by_context_parameters(args)
    print(success, info)
