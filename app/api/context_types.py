...
class DataSource(Enum):
    ...
    def from_str(label: str):
        ...
        raise TypeError("invalid DataSource: " + label)
...
class SearchRequest(object):
    def __init__(self, source: DataSource, terms: List[str]):
        ...
        self.identifier_hash = SearchRequest.make_identifier_hash(
            source, self.terms_string
        )
...
class SearchRequestExt(object):
    ...
    def make_multiple_from_search_terms(search_terms: List[str]) -> List[SearchRequest]:
        return [
            SearchRequest(DataSource.arxiv, search_terms),
            SearchRequest(DataSource.scholar, search_terms),
        ]
...
class SearchContext(object):
    ...
    def set_word_document_state(
        self, state: WordDocumentState, word_document_info: str
    ):
    ...
    def friendly_name(self) -> str:
        return (
            self.search_request.identifier_hash
            + "/"
            + self.search_request.source_name
            + ", "
            + self.search_request.terms_string
        )
    ...
    def serialize_json(self):
        ...
        with open(filepath, "w", encoding="utf-8") as file_handle:
            file_handle.write(
                json.dumps(self, indent=2, cls=JsonEncoder, ensure_ascii=False)
            )
...
class SearchContextExt:
    @staticmethod
    def deserialize_from_filename(filename: str) -> Union[SearchContext, None]:
    ...
    @staticmethod
    def deserialize_from_identifier_hash(
        identifier_hash: str,
    ) -> Union[SearchContext, None]:
    ...
    def get_search_contexts() -> List[SearchContext]:
        ...
        for item in pickles:
            context = SearchContextExt.deserialize_from_filename(item)
